...
class ETWCollector
{
    public static void StartTrace(CollectorType CollectorType, ulong TraceKeywords, OutputType OutputType, String Path, FilterOption FilterOption, Object FilterValue, String ProviderName = "", UserTraceEventLevel UserTraceEventLevel = UserTraceEventLevel.Informational)
    {
        ...
        using (var TraceSession = new TraceEventSession(SilkUtility.EventParseSessionName))
        {
            ...
            using (var EventSource = new ETWTraceEventSource(SilkUtility.EventParseSessionName, TraceEventSourceType.Session))
            {
                ...
                SilkUtility.SetupCtrlCHandler(() =>
                {
                    ...
                    TerminateCollector(TraceSession, EventSource);
                });
                ...
                EventParser.All += HandleEvent(OutputType, Path, FilterOption, FilterValue, TraceSession, EventSource, ProviderName);
        ...
        Action<TraceEvent> HandleEvent(OutputType OutputType, string Path, FilterOption FilterOption, object FilterValue, TraceEventSession TraceSession, ETWTraceEventSource EventSource, string providerName)
        {
            return delegate (TraceEvent data)
            {
                ...
                string jsonRecord = "";
                if (providerName == "Microsoft-Windows-DNSServer")
                {
    ...
    private static void TerminateCollector(TraceEventSession TraceSession, ETWTraceEventSource EventSource)
    {
